...
angular.module('service.login', []).service("LoginService", ['$http', '$q', 'apiUrl', 'AuthenticationService', '$rootScope', function ($http, $q, apiUrl, AuthenticationService, $rootScope) {
  ...
  function reAuthenticateUser() {
    return $q(function (resolve, reject) {
      ...
      if (!currentUser) {
        ...
        login(currentUser.service, currentUser.authToken).then(function (response) {
          resolve(response);
        }, function (error) {
          reject("Error");
        });
  ...
  function login(service, token) {
    return $q(function (resolve, reject) {
      ...
      switch (service) {
        case 'facebook':
          Azureservice.login("facebook", token).then(function (results) {
          ...
          break;
        case 'twitter':
          Azureservice.login("twitter", token).then(function (results) {
            var currentUser = Azureservice.getCurrentUser();
            console.log("Current user: " + JSON.stringify(currentUser));
        ...
        default:
          //Google
          Azureservice.login("google", token).then(function (results) {
            var currentUser = Azureservice.getCurrentUser();
            console.log("Current user: " + JSON.stringify(currentUser));
