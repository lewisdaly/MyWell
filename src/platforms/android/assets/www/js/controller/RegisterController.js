...
angular.module('controller.register', []).controller('RegisterController', function ($scope, $location, $rootScope, $ionicModal, $ionicPopup, ApiService, CachingService, apiUrl) {

  var leafletMap = null;
  $scope.resources = ["well", "checkdam", "raingauge"];

  $scope.refreshCoords = function () {
  ...
  var init = function init() {
    ...
    if (angular.isNullOrUndefined(leafletMap)) {
      ...
      L.tileLayer('https://api.mapbox.com/styles/v1/lewisdaly/ciuqhjyzo00242iphq3wo7bm4/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoibGV3aXNkYWx5IiwiYSI6ImNpdXE3ajltaDAwMGYyb2tkdjk2emx3NGsifQ.wnqFweA7kdijEtsgjTJIPw').addTo(leafletMap);
    }
  };
  init();
   ...
   */
  var checkAndUpdateMap = function checkAndUpdateMap() {
  ...
  $scope.submit = function (form) {
    ...
    if (form == null || form.postcode == null || form.owner == null || form.postcode == null || form.id == null || form.type == null || form.lat == null || form.lng == null) {
      isFormValid = false;
    }
    ...
    var villageId = parseInt(form.id.toString()[0]);
    ...
    ApiService.registerWell(data).then(function (response) {
      var alertPopup = $ionicPopup.alert({
        title: 'Thanks!',
